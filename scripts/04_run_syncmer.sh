#!/usr/bin/env bash
# Map reads with syncmer-hifi (default selector) via Docker, recording wall time
# and peak RSS. Builds the lean index once per genome (whichever combo runs
# first), then reuses it.
#
# Usage: 04_run_syncmer.sh <genome> <error_rate> <read_length>
#
# For mapping-only peak RSS (index construction excluded), use the split pair
# 04_index_syncmer.sh + 04_map_syncmer.sh instead.
...
DOCKER="docker run --rm --platform linux/amd64 -v ${BENCH_DIR}:/bench hifi-syncmer-hifi:latest"

# Build the lean index once per genome (new binary default skips L0-L2 → ~0.5 GB)
if [ ! -f "$INDEX_HOST" ]; then
    echo "Building syncmer-hifi lean index for $GENOME (Docker) ..."
    mkdir -p "$INDEX_DIR/$GENOME"
    $DOCKER syncmer-hifi --build-index "$C_REF" "$C_INDEX" --threads "$THREADS"
...
echo "Running syncmer-hifi (Docker): genome=$GENOME err=$ERR len=$LEN"
