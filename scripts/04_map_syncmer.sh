#!/usr/bin/env bash
# Map reads with syncmer-hifi against the PRE-BUILT index, recording wall time
# and the mapping-only peak RSS (index construction excluded — see
# 04_index_syncmer.sh).
#
# Usage: 04_map_syncmer.sh <genome> <error_rate> <read_length>
...
if [ ! -f "$INDEX_HOST" ]; then
    echo "Missing index $INDEX_HOST — run: 04_index_syncmer.sh $GENOME" >&2
    exit 1
fi
if [ -f "$OUT_PAF" ]; then
...
echo "Mapping syncmer-hifi (pre-built index): genome=$GENOME err=$ERR len=$LEN"

python3 "$BENCH_DIR/scripts/run_timed.py" "$METRICS" \
    "${DOCKER_RUN[@]}" hifi-syncmer-hifi:latest \
        syncmer-hifi --map "$C_READS" "$C_INDEX" -o "$C_OUT_PAF" \
            --threads "$THREADS" \
    > /dev/null 2> "$LOG"
